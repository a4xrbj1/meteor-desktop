...
import MeteorApp from './meteorApp.js';
import ElectronBuilder from './electronBuilder.js';
...
class MeteorDesktop {
    ...
    async runPackager() {
        ...
        // Awaited and rethrown rather than logged-and-forgotten: this floating catch meant
        // runPackager resolved while packaging was still running, so the CLI's own handler had
        // nothing to catch and `meteor-desktop package` exited 0 after logging a failed package
        // (seed meteor-desktop-a8f8). Awaiting also makes the command actually wait for the
        // packaging it was asked to do, which a script invoking it has to be able to assume.
        try {
            await this.packager.packageApp();
        } catch (e) {
