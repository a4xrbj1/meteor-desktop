...
class MeteorDesktop {
    ...
    async buildInstaller(throwError = false) {
        ...
        try {
            ...
            if (throwError) {
                // `throw e`, not `throw new Error(e)`. The wrap stringified the original into a new
                // error's message and generated a fresh stack HERE, discarding the one pointing into
                // electron-builder. That was harmless while this catch was unreachable for the
                // electron-builder failure it names - a swallowing catch one frame down meant it
                // never saw an error (seed frontend-4e42) - and it is the operator's only stack now
                // that it is the normal path.
                throw e;
            }
        }
    }
