...
export default class MeteorApp {
    ...
    /**
     * Fetches manifest.json from running project.
     * @returns {Promise.<Object>}
     */
    async acquireManifest() {
        const port = (this.$.env.options.port) ? this.$.env.options.port : 3080;
        const url = `http://127.0.0.1:${port}/manifest.json?meteor_dont_serve_index=true`;
        this.log.info('acquiring manifest.json');
        this.log.debug(`fetching manifest.json from ${url}`);
        const res = await fetch(url);
        this.log.debug(`manifest.json fetch status: ${res.status}`);
        const text = await res.text();
        if (!res.ok) {
            this.log.debug(`manifest.json response body: ${text.substring(0, 500)}`);
            throw new Error(`failed to fetch manifest.json: HTTP ${res.status} from ${url}`);
        }
        const trimmed = text.trim();
        if (!trimmed) {
            throw new Error(`fetched manifest.json from ${url} but got empty response`);
        }
        if (trimmed[0] !== '[' && trimmed[0] !== '{') {
            this.log.debug(`manifest.json response body (non-JSON): ${text.substring(0, 500)}`);
            const preview = JSON.stringify(trimmed.substring(0, 40));
            throw new Error(
                `manifest.json from ${url} is not JSON (starts with: ${preview}). `
                + 'Meteor may be returning HTML. Ensure the app is fully started.'
            );
        }
        try {
            return JSON.parse(text);
        } catch (e) {
            this.log.debug(`manifest.json raw body: ${text.substring(0, 500)}`);
            throw new Error(`manifest.json from ${url} is not valid JSON: ${e.message}`);
        }
    }
    ...
    async copyBuild() {
        ...
        if (this.indexHTMLstrategy === this.indexHTMLStrategies.INDEX_FROM_RUNNING_SERVER) {
            ...
            try {
                ...
                this.log.error('error while trying to download manifest.json for web.browser,'
                    + ' be sure that you are running a meteor server: ', e);
                throw e;
            }
        }
