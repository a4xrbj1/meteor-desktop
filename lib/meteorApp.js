...
export default class MeteorApp {
    ...
    injectEsm() {
        ...
        // PATCH 1: Fix core-runtime.js global object reference
        // In ES modules, `this` at the top level is undefined, so we need to use `window`
        try {
            const files = fs.readdirSync(packagesDir);
            const coreRuntimeFile = files.find((f) => f.startsWith('core-runtime.js'));
            if (coreRuntimeFile) {
                const coreRuntimePath = path.join(packagesDir, coreRuntimeFile);
                let content = fs.readFileSync(coreRuntimePath, 'UTF-8');

                // Replace "var global = this;" with "var global = window;"
                // This is needed because ES modules run in strict mode where `this` is undefined
                if (content.includes('var global = this;')) {
                    content = content.replace(/var global = this;/g, 'var global = window;');
                    fs.writeFileSync(coreRuntimePath, content);
                    this.log.info('patched core-runtime.js: var global = this → var global = window');
                }
            } else {
                this.log.warn('core-runtime.js not found, skipping global object patch');
            }
        } catch (e) {
            this.log.error(`error patching core-runtime.js: ${e.message}`);
        }
