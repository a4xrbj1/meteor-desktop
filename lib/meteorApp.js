...
`;
            ...
            if (activeBuildDir && sourceJs) {
                ...
                try {
                    ...
                    // @meteorjs/rspack 2.x emits TWO chunk URL prefixes:
                    //   /build-chunks/         — static main chunks
                    //   /build-chunks-local/   — dynamic split chunks (the dev
                    //                            server 307-redirects these to
                    //                            /__rspack__/build-chunks-local/)
                    // The skeleton/app.js runtime already whitelists both
                    // prefixes; the build-time scraper must too, otherwise
                    // the asset is referenced in head.html but never copied
                    // to disk and the A2.5 hash-coherence gate throws "missing
                    // on disk" for /build-chunks-local/*.
                    const chunksRefs = (
                        combinedHtmlForScraping.match(/\/(build-chunks(?:-local)?|__rspack__)\/[^"']+/g) || []
                    ).map((u) => u.split('?')[0]);
                    ...
                    for (let i = 0; i < chunksRefs.length; i += 1) {
                        ...
                        if (!fs.existsSync(localPath)) {
                            ...
                            // not to _build/main-prod/. Check public/ as a fallback disk source.
    ...
    async build() {
        ...
        if (!this.$.env.options.skipMobileBuild) {
            ...
            if (fs.existsSync(rspackBuildDir)) {
                const devArtifacts = [
                    path.join(rspackBuildDir, 'main-dev', 'server-meteor.js'),
                    path.join(rspackBuildDir, 'main-dev', 'client-meteor.js'),
                    path.join(rspackBuildDir, 'test', 'server-meteor.js'),
                    path.join(rspackBuildDir, 'test', 'client-meteor.js')
                ].filter((p) => fs.existsSync(p));
                if (devArtifacts.length > 0) {
                    this.log.error('refusing to wipe _build/ — dev-server rspack artifacts present:');
                    ...
                    this.log.error('stop the dev server and remove _build/main-dev and _build/test, then retry.');
                    process.exit(1);
                }
                try {
    ...
    validateManifestAssetCoherence() {
        ...
        // In skipMobileBuild (dev) mode, build-chunks/*.css that contains
        // HTML is expected — it's Meteor's SPA fallback for a path that the
        // rspack dev server (not the Meteor server) serves at runtime. Same
        // pattern as the __rspack__/client-rspack.js placeholder tolerance
        // above. Demote to a warning in dev; remain strict in PROD where the
        // CSS must be a real bundled asset.
