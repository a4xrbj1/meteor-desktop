...
export default class MeteorApp {
    ...
    updateDdpUrl(indexHtml) {
        ...
        // Robust type="module" injection for Meteor 3.x ESM support, excluding cordova.js
    ...
    /**
     * Injects import.meta polyfill for Meteor 3.x ESM support.
     * This polyfills import.meta.url for classic scripts without converting them to ES modules.
     * Meteor's modules.js uses import.meta.url but scripts must run as classic scripts to share globals.
     */
    injectEsm() {
        this.log.info('injecting import.meta polyfill for Meteor 3.x');
        const indexHtml = this.$.env.paths.electronApp.meteorAppIndex;
        let content;
        try {
            content = fs.readFileSync(indexHtml, 'UTF-8');
        } catch (e) {
            this.log.error(`error loading index.html for ESM polyfill: ${e.message}`);
            process.exit(1);
        }

        // Polyfill import.meta for classic scripts - Meteor 3.x uses import.meta.url in modules.js
        // but scripts must remain classic (not type="module") to share the global Package object
        // We cannot use "typeof import.meta" as it causes SyntaxError in non-module scripts
        const polyfillScript = `<script>
// import.meta polyfill for Meteor 3.x in Electron
// Defines import.meta.url for classic scripts that need ESM compatibility
try {
    if (!window.importMetaPolyfilled) {
        Object.defineProperty(window, 'import', {
            value: Object.freeze({ meta: Object.freeze({ url: document.baseURI || location.href }) }),
            writable: false,
            configurable: false
        });
        window.importMetaPolyfilled = true;
    }
} catch (e) {
    // import.meta already exists natively (running as ES module)
}
</script>
`;

        // Insert the polyfill right before the first <script tag
        content = content.replace(/<script\s/i, polyfillScript + '<script ');

        try {
            fs.writeFileSync(indexHtml, content);
        } catch (e) {
            this.log.error(`error writing index.html after ESM polyfill: ${e.message}`);
            process.exit(1);
        }
        this.log.info('import.meta polyfill injected successfully');
    }
