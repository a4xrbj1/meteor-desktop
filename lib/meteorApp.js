        ...
        try {
            ...
            files.forEach((file) => {
                if (file.endsWith('.js')) {
                    ...
                    // Polyfill import.meta — SyntaxError in classic scripts; replace with
                    // an object literal that provides the same .url property.
                    if (content.includes('import.meta')) {
                        content = content.replace(/\bimport\.meta\b/g, '({url: location.href})');
                        patched = true;
                    }
            ...
            if (fs.existsSync(appDir)) {
                ...
                appFiles.forEach((file) => {
                    if (file.endsWith('.js')) {
                        ...
                        // Polyfill import.meta — SyntaxError in classic scripts.
                        if (content.includes('import.meta')) {
                            content = content.replace(/\bimport\.meta\b/g, '({url: location.href})');
                            patched = true;
                        }
...
`;
            ...
            if (missingSrcs.length > 0) {
                // Auto-repair: find root-level JS files on disk not referenced by any script src
                ...
                if (missingSrcs.length === 1 && bundleCandidates.length >= 1) {
                    ...
                    // (Non-bundle files like desktop-hcp.js are already excluded from candidates.)
                    ...
                    if (bundleCandidates.length === 1) {
                        ...
                        if (fs.existsSync(programJsonPath)) {
                            try {
                                ...
                                if (manifestMatches.length === 1) {
                                    ...
                                    this.log.info(
                                        'injectEsm: resolved ambiguous candidates via'
                                        + ` program.json → ${resolvedFromManifest}`
                                    );
                                }
                            } catch (e) {
                                this.log.warn(
                                    'injectEsm: could not parse program.json for'
                        ...
                        if (resolvedFromManifest) {
                            ...
                            this.log.info(
                                `injectEsm: resolved ambiguous candidates by mtime → ${realFile}`
                            );
                        }
                    }
                    ...
                    fs.writeFileSync(indexHtml, htmlContent);
                    this.log.info(`injectEsm: auto-repaired stale hash — replaced ${staleSrc} with ${newSrc}`);
                } else {
    ...
    validateMeteorAsar() {
        ...
        if (!indexFile) {
            ...
            if (!indexHtml.includes('window.setImmediate')) {
                this.log.warn('A3: meteor.asar index.html missing setImmediate polyfill — injectEsm may have failed');
            } else {
            ...
            if (indexHtml.includes('type="module"') || indexHtml.includes("type='module'")) {
                this.log.warn(
                    ...
                    + 'this breaks bare global assignments; injectEsm should not add type=module'
                );
            }
        }
