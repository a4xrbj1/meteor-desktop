...
// Matches any rspack-emitted chunk/asset URL across @meteorjs/rspack@1.x defaults
// (build-chunks/, build-assets/) and @meteorjs/rspack@2.x METEOR_LOCAL_DIR-derived
// suffixes (build-chunks-local/, build-chunks-local-desktop/, build-assets-…).
// Basename class [^/]+ covers every path.basename(METEOR_LOCAL_DIR) the upstream
// algorithm at @meteorjs/rspack/rspack.config.js:247-261 can produce.
const RSPACK_CHUNK_URL_RE = /\/(build-(?:chunks|assets)(?:-[^/]+)?|__rspack__)\/[^"']+/g;
const RSPACK_CHUNK_PATH_RE = /^build-(?:chunks|assets)(?:-[^/]+)?\//;
...
export default class MeteorApp {
    ...
    async checkPreconditions() {
        ...
        if (!this.$.env.options.skipMobileBuild
            && this.indexHTMLstrategy === this.indexHTMLStrategies.INDEX_FROM_LOCAL_BUILD) {
    ...
    async acquireIndex() {
        ...
        if (/<title>\s*Meteor App\s*[-—]\s*Error\s*<\/title>/i.test(text)) {
            const logMatch = text.match(/<code[^>]*class="[^"]*log-content[^"]*"[^>]*>([\s\S]*?)<\/code>/i);
            const meteorErr = logMatch
                ? logMatch[1]
                    .replace(/&quot;/g, '"').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>')
                    .trim()
                : '(could not parse Meteor error log from response)';
            throw new Error(
                `meteor server at ${url} returned its "Meteor App - Error" page — refusing to pack the error HTML.\n`
                + `meteor reported:\n${meteorErr}\n`
                + 'fix the meteor build error first, then retry.'
            );
        }
        return text;
    }
    ...
    buildMobileTarget() {
        ...
        return new Promise((resolve, reject) => {
            ...
            // Lets spawn meteor. Inherit process.env first, then layer the
            // explicit overrides on top — the reverse order (Object.assign(env,
            // process.env)) silently undoes NODE_ENV=production whenever the
            // parent shell has its own NODE_ENV, see seed meteor-desktop-7691.
            const child = spawn(
                'meteor',
                args,
                {
    ...
    /**
     * A2.5: Hash coherence gate — validates that every script src and stylesheet <link> href
     * in index.html resolves to a file on disk. Runs auto-repair (program.json lookup then
     * mtime fallback) when a stale hash is detected. Must run after validateBundleStructure
     * (A2) and before injectEsm.
     *
    ...
    *   development builds, because they bootstrap the real client JavaScript bundle.
     ...
     * - Rewrites unhashed rspack stylesheet <link> hrefs (e.g. /build-chunks-local-desktop/
     *   main.css) to their content-hashed manifest URL, and prunes stylesheet links that have
     *   no bundled asset at all (foreign non-desktop build-context artifacts).
     *
     * @throws {Error} if any script src is missing and cannot be auto-repaired, or if every
     *   stylesheet <link> is unresolvable
     */
    validateHashCoherence() {
        ...
        // rspack content-hashes the main CSS (emitted as type:"asset", not "css") and places it
        // under app/, but Meteor's head.html references it unhashed at the web root. Index every
        // manifest CSS asset by its unhashed url so an unhashed <link> href can be matched to its
        // content-hashed bundled asset further down.
        const cssByUnhashedUrl = new Map();
        if (fs.existsSync(programJsonPath)) {
            try {
                ...
                (programJson.manifest || []).forEach((entry) => {
                    ...
                    const cssUrl = entry.url.split('?')[0];
                    const hashed = cssUrl.match(/^(.*\/[^/]+)\.[0-9a-f]{8,}\.css$/);
                    if (hashed) {
                        const unhashedUrl = `${hashed[1]}.css`;
                        if (!cssByUnhashedUrl.has(unhashedUrl)) {
                            cssByUnhashedUrl.set(unhashedUrl, []);
                        }
                        cssByUnhashedUrl.get(unhashedUrl).push({ url: cssUrl, path: entry.path });
                    }
                });
            } catch (e) {
        ...
        // Normalize <link rel="stylesheet"> hrefs to manifest-coherent URLs. rspack emits the
        // main CSS content-hashed under app/ while Meteor's head.html references it unhashed at
        // the web root; the runtime AssetHandler resolves assets by exact manifest URL, so each
        // stylesheet href must be rewritten to its hashed manifest URL. A stylesheet link with
        // no bundled asset at all (a foreign non-desktop build-context artifact) is pruned.
        const stylesheetLinkRegex = /<link\b[^>]*\brel="stylesheet"[^>]*>/gi;
        const rspackCssUrlRe = /^\/build-(?:chunks|assets)(?:-[^/]+)?\//;
        const stylesheetTags = htmlContent.match(stylesheetLinkRegex) || [];
        const cssLinksToPrune = [];
        let resolvableStylesheetCount = 0;
        stylesheetTags.forEach((tag) => {
            ...
            if (hrefPath.startsWith('http') || hrefPath.startsWith('//')) {
                resolvableStylesheetCount += 1;
                return;
            }
            const hrefKey = hrefPath.replace(/^\//, '');
            if (fs.existsSync(path.join(meteorAppDir, hrefKey))
                || fs.existsSync(path.join(meteorAppDir, 'app', hrefKey))) {
                resolvableStylesheetCount += 1;
                return;
            }
            // Only rspack-style build-chunks/build-assets CSS gets rewritten or pruned; any
            // other unresolved stylesheet is left for injectEsm's validator to report.
            if (!rspackCssUrlRe.test(hrefPath)) {
                return;
            }
            const siblings = cssByUnhashedUrl.get(hrefPath) || [];
            if (siblings.length > 0) {
                let [chosen] = siblings;
                if (siblings.length > 1) {
                    // Multiple hashed candidates (stale duplicates) — prefer the one whose file
                    // exists, newest by mtime, mirroring the script disambiguation below.
                    chosen = siblings
                        .filter((e) => fs.existsSync(path.join(meteorAppDir, e.path)))
                        .reduce((newest, e) => {
                            if (!newest) {
                                return e;
                            }
                            const newestMtime = fs.statSync(path.join(meteorAppDir, newest.path)).mtimeMs;
                            const candidateMtime = fs.statSync(path.join(meteorAppDir, e.path)).mtimeMs;
                            return candidateMtime > newestMtime ? e : newest;
                        }, null) || siblings[0];
                }
                const newTag = tag.replace(/\bhref="[^"]+"/i, `href="${chosen.url}"`);
                htmlContent = htmlContent.split(tag).join(newTag);
                htmlWasUpdated = true;
                resolvableStylesheetCount += 1;
                this.log.info(`A2.5: resolved stylesheet ${href} → ${chosen.url} (content-hash)`);
                return;
            }
            cssLinksToPrune.push({ tag, href });
        });
        if (cssLinksToPrune.length > 0 && resolvableStylesheetCount === 0) {
            // In skipMobileBuild (dev) mode every cssLinksToPrune entry already matches
            // rspackCssUrlRe (filter at line 1109). At runtime the asar's AssetHandler in
            // skeleton/app.js:770-779 intercepts /build-chunks-*/* requests and routes them
            // via net.fetch to the running dev server, which 307s them to rspack-dev-server's
            // in-memory bundle — so an on-disk miss is by design, not a packaging bug. Same
            // dev-mode tolerance pattern already applied below for the __rspack__/client-rspack.js
            // placeholder (~line 2440) and CSS-content HTML check (~line 2502). Demote the
            // throw to a warning and keep the links in index.html so the runtime proxy can
            // deliver the real CSS.
            if (this.$.env.options.skipMobileBuild) {
                this.log.warn(
                    'A2.5: dev-mode stylesheet(s) unresolvable on disk — served at runtime'
                    + ` by the rspack dev server via the AssetHandler proxy:\n${
                        cssLinksToPrune.map((c) => `  ${c.href}`).join('\n')}`
                );
                cssLinksToPrune.length = 0;
            } else {
                const prunedList = cssLinksToPrune.map((c) => `  ${c.href}`).join('\n');
                throw new Error(
                    'A2.5: every <link rel="stylesheet"> in index.html is unresolvable — refusing'
                    + ` to package a style-less desktop build:\n${prunedList}`
                );
            }
        }
        cssLinksToPrune.forEach(({ tag, href }) => {
            htmlContent = htmlContent.split(tag).join('');
            htmlWasUpdated = true;
            this.log.info(
                `A2.5: pruned stale stylesheet link with no bundled asset: ${href} (foreign build context)`
            );
        });
        ...
        // mongo-dev-server.js is a server-only package that Meteor generates a script tag
        // for in index.html, but it doesn't produce a JS file in the web.browser build.
        ...
        allSrcMatches.forEach((m) => {
            ...
            if (SERVER_ONLY_PACKAGES.has(srcPath)) {
                this.log.info(`A2.5: skipping server-only package script: ${src}`);
                return;
            }
...
`;
            ...
            const rspackCtx = this.$.env.paths.meteorApp.rspack;

            // Probe v2.x-preferred (METEOR_LOCAL_DIR-derived) AND v1.x fallback contexts.
            // First found wins. Dedupe in case rspackCtx.buildContext === '_build'
            // (v1.x defaults). See rspack.config.js:247-261 for the upstream algorithm.
            const candidateBuildDirs = [...new Set([rspackCtx.buildContext, '_build'])]
                .map((ctx) => path.join(projectRoot, ctx));
            const buildTypes = ['main-prod', 'main-dev'];
            let activeBuildDir = null;
            let sourceJs = null;

            for (let i = 0; i < candidateBuildDirs.length && !sourceJs; i += 1) {
                for (let j = 0; j < buildTypes.length; j += 1) {
                    const candidate = path.join(candidateBuildDirs[i], buildTypes[j], 'client-rspack.js');
                    if (fs.existsSync(candidate)) {
                        activeBuildDir = path.join(candidateBuildDirs[i], buildTypes[j]);
                        sourceJs = candidate;
                        break;
                    }
                }
            ...
            if (!activeBuildDir) {
                const probedList = candidateBuildDirs
                    .map((d) => `${d}/{main-prod,main-dev}/client-rspack.js`)
                    .join(', ');
                throw new Error(
                    `injectEsm: no rspack client bundle found. Probed: ${probedList}. `
                    + 'If you set Meteor.buildContext or RSPACK_BUILD_CONTEXT, export it '
                    + 'before invoking meteor-desktop.'
                );
            }

            {
                ...
                try {
                    ...
                    // rspack v2.x (rspack.config.js:247-261) derives chunk/asset
                    // directory names by suffixing path.basename(METEOR_LOCAL_DIR):
                    // dev sees '-local', prod desktop sees '-local-desktop'. v1.x
                    // consumers emit the bare 'build-chunks/' name. RSPACK_CHUNK_URL_RE
                    // admits both shapes plus the canonical '__rspack__/' prefix so
                    // every scraped asset is copied to disk and survives the A2.5
                    // hash-coherence gate.
                    // Index the Meteor bundle's program.json: every manifest url (assets
                    // already bundled under app/ and served by the runtime AssetHandler) and a
                    // map from unhashed CSS url → its content-hashed sibling. rspack's
                    // index.html references the main CSS unhashed but it is bundled hashed.
                    const bundledManifestUrls = new Set();
                    const cssByUnhashedUrl = new Map();
                    const pjForChunks = path.join(meteorAppDir, 'program.json');
                    if (fs.existsSync(pjForChunks)) {
                        try {
                            const pjm = JSON.parse(fs.readFileSync(pjForChunks, 'utf8'));
                            (pjm.manifest || []).forEach((e) => {
                                if (!e.url) {
                                    return;
                                }
                                const mUrl = e.url.split('?')[0];
                                bundledManifestUrls.add(mUrl);
                                const hashed = mUrl.match(/^(.*\/[^/]+)\.[0-9a-f]{8,}\.css$/);
                                if (hashed) {
                                    const unhashedUrl = `${hashed[1]}.css`;
                                    if (!cssByUnhashedUrl.has(unhashedUrl)) {
                                        cssByUnhashedUrl.set(unhashedUrl, []);
                                    }
                                    cssByUnhashedUrl.get(unhashedUrl).push(mUrl);
                                }
                            });
                        } catch (e) {
                            this.log.warn(`injectEsm: could not parse program.json for chunk skip-set: ${e.message}`);
                        }
                    }

                    // Resolve scraped chunk URLs. rspack's index.html references the main CSS
                    // unhashed (/build-chunks-local-desktop/main.css) — rewrite it to the
                    // content-hashed manifest url so the scraper, the A2.7 gate, the injected
                    // <link> tag and the runtime AssetHandler all point at the real bundled
                    // asset under app/. Drop a CSS chunk url that resolves to no asset at all.
                    const chunksRefs = (combinedHtmlForScraping.match(RSPACK_CHUNK_URL_RE) || [])
                        .map((u) => u.split('?')[0])
                        .map((u) => {
                            if (!u.endsWith('.css') || bundledManifestUrls.has(u)) {
                                return u;
                            }
                            const siblings = cssByUnhashedUrl.get(u);
                            if (siblings && siblings.length > 0) {
                                this.log.info(`injectEsm: resolved css chunk ${u} → ${siblings[0]} (content-hash)`);
                                return siblings[0];
                            }
                            const rel = u.replace(/^\//, '');
                            if (fs.existsSync(path.join(meteorAppDir, rel))
                                || fs.existsSync(path.join(meteorAppDir, 'app', rel))) {
                                return u;
                            }
                            this.log.warn(`injectEsm: dropping unresolvable css chunk url ${u} (no bundled asset)`);
                            return null;
                        })
                        .filter((u) => u !== null);
                    ...
                    for (let i = 0; i < chunksRefs.length; i += 1) {
                        ...
                        const cleaned = chunkUrl.replace(/^\//, '');
                        const localPath = path.join(meteorAppDir, cleaned);
                        const appLocalPath = path.join(meteorAppDir, 'app', cleaned);
                        const diskSource = path.join(activeBuildDir, cleaned);

                        // bundledManifestUrls covers the URL-registered case (added by v6.0.8);
                        // appLocalPath covers an orphan-bundled file at app/<rel> whose URL
                        // is absent from program.json but admitted by the chunksRefs validator
                        // above. Together they ensure no redundant root-level copy regardless
                        // of how the file reached app/.
                        if (!bundledManifestUrls.has(chunkUrl)
                            && !fs.existsSync(localPath)
                            && !fs.existsSync(appLocalPath)) {
                            if (!fs.existsSync(path.dirname(localPath))) {
                                fs.mkdirSync(path.dirname(localPath), { recursive: true });
                            }

                            // Rspack writes CSS/JS chunks to public/ (client output dir),
                            // not to the resolved buildContext (e.g. _build/, _build-local-desktop/).
                            // Check public/ as a fallback disk source.
                            const publicSource = path.join(projectRoot, 'public', cleaned);
                            ...
                            if (fs.existsSync(diskSource)) {
                                ...
                                fs.copyFileSync(publicSource, localPath);
                                this.log.info(
                                    `injectEsm: copied chunk from public/: ${chunkUrl}`
                                );
                                downloadedChunks.add(chunkUrl);
                            } else {
                                ...
                                // Meteor's dev server returns HTTP 200 + text/html (a SPA
                                // error page) for paths it does not serve. Writing that
                                // body under a .css/.js path silently contaminates the
                                // asar. A3.5 Check 4 (~lines 2438-2491) catches it
                                // post-hoc via magic-byte scan; this pre-flight guard
                                // keeps the bad bytes off disk and produces a precise
                                // port + content-type diagnostic on rejection.
                                let expectedCt = null;
                                if (chunkUrl.endsWith('.css')) {
                                    expectedCt = 'css';
                                } else if (chunkUrl.endsWith('.js')) {
                                    expectedCt = 'javascript';
                                }
                                    ...
                                    try {
                                        ...
                                        if (res.ok) {
                                            ...
                                            if (expectedCt && !ct.includes(expectedCt)) {
                                                this.log.warn(
                                                    `injectEsm: ${fullUrl} returned content-type "${ct}"`
                                                    + ` — expected ${expectedCt}; not writing to ${localPath}`
                                                );
                    ...
                    // Assets already in the program.json manifest are bundled (served from app/
                    // by the runtime AssetHandler) — skip them; only genuinely absent assets fail.
                    chunksRefs.forEach((u) => {
                        if (bundledManifestUrls.has(u)) {
                            return;
                        }
                        const rel = u.replace(/^\//, '');
                        if (fs.existsSync(path.join(meteorAppDir, rel))
                    ...
                    // 3. Inject the rspack main bundle as an eager <script> ONLY when the Meteor
                    // bundle does not already link it. Under @meteorjs/rspack v2 the Meteor bundle
                    // (app/app.js) bundles the client-rspack graph as a module AND executes it via
                    // `module.link('./client-rspack.js')` — web prod ships app.js alone and runs the
                    // whole graph. A second <script src="/__rspack__/client-rspack.js"> then
                    // re-evaluates the ENTIRE client bundle: duplicate DDP logins, duplicate Tracker
                    // autoruns, and the Login component remounting to the first step after the
                    // security token is pasted (incident 2026-07-10, yourdna.family frontend). Detect
                    // the link in the on-disk Meteor bundle and skip the redundant tag; the
                    // client-rspack.js file, its lazy chunks and manifest entry stay for chunk
                    // loading (and the A3.5 real-bundle gate). If the Meteor bundle does NOT link it
                    // (older pipelines where app.js is a thin shim), inject the tag as before.
                    const meteorBundlePaths = [
                        path.join(meteorAppDir, 'app', 'app.js'),
                        path.join(meteorAppDir, 'app.js')
                    ];
                    let meteorBundleLinksRspack = false;
                    for (let bi = 0; bi < meteorBundlePaths.length && !meteorBundleLinksRspack; bi += 1) {
                        const bundlePath = meteorBundlePaths[bi];
                        if (fs.existsSync(bundlePath)
                            && /module\.link\((['"])\.\/client-rspack\.js\1/.test(fs.readFileSync(bundlePath, 'utf8'))) {
                            meteorBundleLinksRspack = true;
                        }
                    }
                    const tag = '<script src="/__rspack__/client-rspack.js"></script>';
                    if (meteorBundleLinksRspack) {
                        this.log.info(
                            'injectEsm: skipped redundant /__rspack__/client-rspack.js <script> — '
                            + 'Meteor bundle already links the graph via module.link'
                        );
                    } else if (!htmlContent.includes('/__rspack__/client-rspack.js')) {
                    ...
                    if (fs.existsSync(pjPath)) {
                        ...
                        allRspackUrls.forEach((u) => {
                            ...
                            // Probe root-level first, then app/<rel>: the scraper write
                            // block above skips when an app/-relative copy already exists,
                            // so for those URLs the authoritative on-disk file lives under
                            // app/ and the manifest entry must point there.
                            const cleanedU = u.replace(/^\//, '');
                            const rootCandidate = path.join(meteorAppDir, cleanedU);
                            const appCandidate = path.join(meteorAppDir, 'app', cleanedU);
                            let resolvedPath = null;
                            if (fs.existsSync(rootCandidate)) {
                                resolvedPath = rootCandidate;
                            } else if (fs.existsSync(appCandidate)) {
                            ...
                            pj.manifest.push({
                                path: path.relative(meteorAppDir, resolvedPath),
                                where: 'client',
            ...
            // mongo-dev-server.js is a server-only package — skip it.
            const SERVER_ONLY_PACKAGES = new Set([
                '/packages/mongo-dev-server.js',
            ]);
            ...
            allSrcMatches.forEach((m) => {
                ...
                if (SERVER_ONLY_PACKAGES.has(srcPath)) {
                    this.log.info(`injectEsm: skipping server-only package script: ${src}`);
                    return;
                }
            ...
            allHrefMatches.forEach((m) => {
                ...
                // In skipMobileBuild (dev) mode, /build-chunks-*/* and /__rspack__/* links
                // are served at runtime by the rspack dev server via the AssetHandler proxy
                // (skeleton/app.js:770-779) — not on disk by design, mirroring A2.5's
                // dev-mode tolerance above. Same accommodation as the script-src loop
                // already grants /__rspack__/ unconditionally at line 1966.
                if (this.$.env.options.skipMobileBuild
                    && (RSPACK_CHUNK_PATH_RE.test(hrefPath.replace(/^\//, ''))
                        || hrefPath.startsWith('/__rspack__/'))) {
                    this.log.info(`injectEsm: allowing dev-mode runtime-resolved link: ${href}`);
                    return;
                }
    ...
    async build() {
        ...
        // Clean every candidate rspack build dir to prevent stale dev artifacts from
        // contaminating the prod build. Under @meteorjs/rspack@^2.x the dir name is
        // METEOR_LOCAL_DIR-derived (e.g. _build-local-desktop/), under @1.x it is the
        // fixed _build/; iterate both so v1→v2 transitions don't orphan a stale tree.
        // Meteor's production build regenerates <buildContext>/main-prod/ fresh during
        // buildMobileTarget().
        // IMPORTANT: Only clean when we will run buildMobileTarget() (i.e. skipMobileBuild
        // is false). In skipMobileBuild mode the running dev server owns
        // <buildContext>/main-dev/ — deleting it crashes the dev server and leaves
        // copyBuild() with a broken/incomplete web.browser output.
        if (!this.$.env.options.skipMobileBuild) {
            const { rspack } = this.$.env.paths.meteorApp;
            const candidateDirs = [...new Set([rspack.buildContext, '_build'])]
                .map((ctx) => ({ ctx, dir: path.join(this.$.env.paths.meteorApp.root, ctx) }))
                .filter(({ dir }) => fs.existsSync(dir));
            for (let i = 0; i < candidateDirs.length; i += 1) {
                const { ctx, dir } = candidateDirs[i];
                const devArtifacts = [
                    path.join(dir, 'main-dev', 'server-meteor.js'),
                    path.join(dir, 'main-dev', 'client-meteor.js'),
                    path.join(dir, 'test', 'server-meteor.js'),
                    path.join(dir, 'test', 'client-meteor.js')
                ].filter((p) => fs.existsSync(p));
                if (devArtifacts.length > 0) {
                    this.log.error(`refusing to wipe ${ctx}/ — dev-server rspack artifacts present:`);
                    devArtifacts.forEach((p) => {
                        this.log.error(`  ${path.relative(this.$.env.paths.meteorApp.root, p)}`);
                    });
                    this.log.error('a parallel `meteor run` (or `npm run start`) would be crashed by the wipe.');
                    this.log.error(`stop the dev server and remove ${ctx}/main-dev and ${ctx}/test, then retry.`);
                    process.exit(1);
                }
                try {
                    // eslint-disable-next-line no-await-in-loop
                    await this.$.utils.rmWithRetries('-rf', dir);
                    this.log.info(`cleaned ${ctx}/ directory to prevent stale rspack artifacts`);
                } catch (e) {
                    this.log.warn(`failed to clean ${ctx}/ directory: ${e.message} — continuing build`);
                }
            }
        }
        ...
        try {
            this.validateRuntimeConfigUrls();
        } catch (e) {
            this.log.error(`A2.6 runtime-config URL validation failed: ${e.message}`);
            throw e;
        }
        this.$.electronApp.validationGatesPassed.push('A2.6 runtime-config URLs');
    ...
    /**
     * A2.6: Validates that the staged index.html ships with a deployable
     * ROOT_URL before packToAsar seals it into meteor.asar. Catches the
     * 'http://build/' placeholder Meteor emits when the inner 'meteor run'
     * was contended by a leftover meteor-desktop or rspack watcher and
     * acquireIndex() got back half-initialised HTML (seed e2e-4193).
     *
     * When --ddpUrl was supplied (--build-meteor path), additionally asserts
     * both ROOT_URL and DDP_DEFAULT_CONNECTION_URL exactly equal the
     * configured ddpUrl — updateDdpUrl@806-807 writes both, so a mismatch
     * means the rewrite was incomplete.
     *
     * When --ddpUrl is null (dev-mode npm run desktop path), DDP_DEFAULT_CONNECTION_URL
     * is allowed to be absent from the serialised __meteor_runtime_config__ —
     * Meteor omits it when no --server was passed to 'meteor run', and
     * skeleton/app.js:953-960 injects an override script at request time that
     * sets both keys to http://127.0.0.1:{meteorDevPort}/. The runtime layer
     * recovers the absence; the gate must not treat it as an error.
     *
     * Pairs with the downstream check in e2e/global-setup.js:212-243 that
     * reads packed asar bytes. Both layers coexist: A2.6 covers everything
     * through MeteorApp.build(); the e2e gate covers asars produced by older
     * meteor-desktop versions or out-of-band tooling.
     *
     * @throws {Error} if ROOT_URL is missing, if any present URL is
     *   unparseable / has hostname 'build', or (when --ddpUrl was supplied)
     *   if either URL is absent or does not match the configured ddpUrl.
     */
    validateRuntimeConfigUrls() {
        const indexPath = this.$.env.paths.electronApp.meteorAppIndex;
        let content;
        try {
            content = fs.readFileSync(indexPath, 'utf8');
        } catch (e) {
            throw new Error(`A2.6: cannot read index.html at ${indexPath}: ${e.message}`);
        }
        const matches = content.match(this.matcher);
        if (!matches) {
            throw new Error(`A2.6: __meteor_runtime_config__ not found in ${indexPath}`);
        }
        let runtimeConfig;
        try {
            runtimeConfig = JSON.parse(decodeURIComponent(matches[1]));
        } catch (e) {
            throw new Error(`A2.6: __meteor_runtime_config__ in ${indexPath} is not valid JSON: ${e.message}`);
        }
        const configuredDdpUrl = this.$.env.options.ddpUrl;
        let expectedUrl = null;
        if (configuredDdpUrl) {
            expectedUrl = configuredDdpUrl.substr(-1, 1) === '/' ? configuredDdpUrl : `${configuredDdpUrl}/`;
        }
        const fail = (reason, isBuildPlaceholder) => {
            const hint = isBuildPlaceholder
                ? '\nLikely cause: meteor-desktop\'s inner \'meteor run\' was contended by a leftover '
                  + 'meteor-desktop or rspack watcher from a prior incomplete build. Kill those '
                  + '(ps -axo pid,command | grep -E "meteor-desktop|rspack") and rebuild.'
                : '';
            throw new Error(
                `A2.6 runtime-config URL gate failed: ${reason}\n`
                + `  indexHtml:                  ${indexPath}\n`
                + `  ROOT_URL:                   ${runtimeConfig.ROOT_URL}\n`
                + `  DDP_DEFAULT_CONNECTION_URL: ${runtimeConfig.DDP_DEFAULT_CONNECTION_URL}\n`
                + `  configured --ddpUrl:        ${configuredDdpUrl || '(none)'}${
                    hint}`
            );
        };
        if (typeof runtimeConfig.ROOT_URL !== 'string' || !runtimeConfig.ROOT_URL) {
            fail('ROOT_URL is missing or empty', false);
        }
        ['ROOT_URL', 'DDP_DEFAULT_CONNECTION_URL'].forEach((key) => {
            const value = runtimeConfig[key];
            const isPresent = typeof value === 'string' && value.length > 0;
            if (!isPresent) {
                if (expectedUrl !== null) {
                    fail(`${key} is missing or empty (expected ${expectedUrl} via --ddpUrl)`, false);
                }
                return;
            }
            let url;
            try {
                ...
                fail(`${key} is not a parseable URL: ${e.message}`, false);
            }
            if (url.hostname === 'build') {
                fail(`${key} hostname is 'build' (Meteor's "no --server provided" placeholder)`, true);
            }
            if (expectedUrl !== null && value !== expectedUrl) {
                fail(`${key}=${value} does not match configured --ddpUrl=${expectedUrl}`, false);
            }
        });
        this.log.info(`A2.6: runtime-config URLs OK (ROOT_URL=${runtimeConfig.ROOT_URL})`);
    }
    ...
    validateManifestAssetCoherence() {
        ...
        [...scriptSrcs, ...linkHrefs].forEach((ref) => {
            ...
            // In skipMobileBuild (dev) mode, /build-chunks-*/* and /__rspack__/* references
            // are served at runtime by the rspack dev server via the AssetHandler proxy
            // (skeleton/app.js:770-779). They have no program.json manifest entry by design;
            // the runtime proxy bypasses AssetHandler's manifest lookup. Same dev-mode
            // tolerance as A2.5 (~line 1138) and injectEsm's link validator (~line 2001).
            if (this.$.env.options.skipMobileBuild
                && (RSPACK_CHUNK_PATH_RE.test(refPath) || refPath.startsWith('__rspack__/'))) {
                this.log.info(`A3.5: allowing dev-mode runtime-resolved ref: /${refPath}`);
                return;
            }
            ...
            if (isPlaceholder || tooSmall) {
                if (this.$.env.options.skipMobileBuild) {
                    ...
                    throw new Error(
                        ...
                        + ` — stale ${rspackCtx.buildContext}/main-dev/ was used`
                        + ' instead of main-prod/'
                    );
                }
            } else {
                this.log.info(
        ...
        // For each <link href> pointing to a build-chunks*/ path (v1.x defaults
        // OR @meteorjs/rspack@^2.x METEOR_LOCAL_DIR-derived suffixes), verify
        // the file contains CSS and not HTML or JS served under a .css extension.
        // In skipMobileBuild (dev) mode, build-chunks*/*.css that contains HTML
        // is expected — it's Meteor's SPA fallback for a path that the rspack
        // dev server (not the Meteor server) serves at runtime. Same pattern as
        // the __rspack__/client-rspack.js placeholder tolerance above. Demote
        // to a warning in dev; remain strict in PROD.

        const cssErrors = [];
        linkHrefs.forEach((href) => {
            const hrefPath = href.split('?')[0].replace(/^\//, '');
            // Chunks-only (not assets) and CSS-only — match v1.x 'build-chunks/'
            // AND any v2.x 'build-chunks-<suffix>/'. Asset shapes are excluded
            // by re-asserting the 'build-chunks' literal prefix.
            if (!hrefPath.startsWith('build-chunks')
                || !RSPACK_CHUNK_PATH_RE.test(hrefPath)
                || !hrefPath.endsWith('.css')) {
                return;
            }
            // Resolve through the manifest so app/-bundled CSS is also validated:
            // the runtime AssetHandler reads the asar at entry.path, so that is
            // the file whose content this check must inspect.
            const manifestEntry = manifestByUrl.get(hrefPath);
            const asarFile = manifestEntry && manifestEntry.path
                ? manifestEntry.path
                : hrefPath;
            if (!allFilesSet.has(asarFile)) {
                return; // Already caught by Check 2
            }
            const content = asar.extractFile(asarPath, asarFile)
        ...
        if (cssErrors.length > 0) {
            if (this.$.env.options.skipMobileBuild) {
                this.log.warn(
                    'A3.5: build-chunks CSS file(s) contain HTML — expected in dev mode,'
                    + ` real CSS served by rspack dev server at runtime:\n${
                        cssErrors.join('\n')}`
                );
            } else {
                throw new Error(
