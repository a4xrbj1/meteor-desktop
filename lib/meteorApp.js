...
export default class MeteorApp {
    ...
    injectEsm() {
        ...
        try {
            ...
            files.forEach(file => {
                if (file.endsWith('.js')) {
                    ...
                    // Use regex to catch optional whitespace
                    ...
                    // Many packages (underscore, etc) use (function(){...}).call(this)
                    // In ESM `this` is undefined, so we fallback to window.
            ...
            if (patchedCount > 0) {
                this.log.info(`patched ${patchedCount} files in packages/: var global = this → var global = window`);
            } else {
                this.log.warn('no files found requiring global patch in packages/ directory');
            }

            // PATCH: Fix global-imports.js (ESM Strict Mode)
            // Replace `Mongo = Package.mongo.Mongo` with `window.Mongo = Package.mongo.Mongo`
            const globalImportsPath = path.join(meteorAppDir, 'app', 'global-imports.js');
            if (fs.existsSync(globalImportsPath)) {
                let giContent = fs.readFileSync(globalImportsPath, 'UTF-8');
                // Use a regex that matches start of line assignments to Package
                // Matches "Name = Package..." -> "window.Name = Package..."
                if (!giContent.includes('window.Meteor =')) {
                    giContent = giContent.replace(/^([a-zA-Z0-9_$]+)\s*=\s*Package\./gm, 'window.$1 = Package.');
                    fs.writeFileSync(globalImportsPath, giContent);
                    this.log.info('patched global-imports.js: Name = Package... → window.Name = Package...');
                }
            }
        ...
        try {
            ...
            // Add type="module" to script tags with src attribute, excluding cordova.js
            // First, handle scripts with type="text/javascript"
            htmlContent = htmlContent.replace(
                /<script type="text\/javascript" src="([^"]+)"/g,
                (match, src) => {
                    if (src.includes('cordova.js')) {
                        return match; // Keep cordova.js as-is
                    }
