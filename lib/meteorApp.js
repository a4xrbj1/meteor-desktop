...
export default class MeteorApp {
        ...
        if (this.$.desktop.getSettings().desktopHCP) {
            ...
            try {
                if (!this.meteorManager.checkPackages(desktopHCPPackages)) {
                    this.log.warn('desktopHCP requires local packages, adding them now');
                    await this.meteorManager.addPackages(desktopHCPPackages, desktopHCPPackages);
                }
            } catch (e) {
                throw new Error(e);
            }
    ...
    async acquireIndex() {
        ...
        const url = `http://127.0.0.1:${port}/__cordova/index.html`;
        ...
        // Verify we got the web.cordova build index (must include cordova.js).
        if (!text.includes('src="/cordova.js"')) {
            this.log.debug(`index.html response body (cordova.js not found): ${text.substring(0, 500)}`);
            throw new Error(
                `fetched index.html from ${url} but it does not contain cordova.js script tag`
                + ' - ensure meteor is running with --mobile-server'
            );
        }
        return text;
    }
     ...
     */
    buildMobileTarget() {
        const programJson = (this.indexHTMLstrategy === this.indexHTMLStrategies.INDEX_FROM_CORDOVA_BUILD)
