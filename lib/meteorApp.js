...
export default class MeteorApp {
    ...
    /**
     * Patches Meteor 3.x ESM compatibility issue.
     * Meteor 3.x uses import.meta.url in modules.js, which causes SyntaxError in classic scripts.
     * This method replaces import.meta.url with a compatible alternative before packing.
     */
    injectEsm() {
        this.log.info('patching Meteor 3.x import.meta.url for Electron compatibility');
        const meteorAppDir = this.$.env.paths.electronApp.meteorApp;
        const packagesDir = path.join(meteorAppDir, 'packages');

        // Find and patch modules.js which contains import.meta.url
        let modulesJsPath;
        try {
            const files = fs.readdirSync(packagesDir);
            const modulesFile = files.find(f => f.startsWith('modules.js'));
            if (modulesFile) {
                modulesJsPath = path.join(packagesDir, modulesFile);
            }
        } catch (e) {
            this.log.warn(`could not read packages directory: ${e.message}`);
            return;
        }

        if (!modulesJsPath) {
            this.log.warn('modules.js not found in packages directory, skipping ESM patch');
            return;
        }

        try {
            let content = fs.readFileSync(modulesJsPath, 'UTF-8');

            // Replace import.meta.url with document.baseURI (compatible with classic scripts)
            // The original code: configure({ baseURI: import.meta.url });
            // import.meta.url is a syntax feature that causes SyntaxError in non-module contexts
            if (content.includes('import.meta.url')) {
                content = content.replace(
                    /import\.meta\.url/g,
                    '(document.baseURI || location.href)'
                );
                fs.writeFileSync(modulesJsPath, content);
                this.log.info('successfully patched import.meta.url in modules.js');
            } else {
                this.log.info('no import.meta.url found in modules.js, no patch needed');
            }
        } catch (e) {
            this.log.error(`error patching modules.js: ${e.message}`);
        }
    }
