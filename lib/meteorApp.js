...
export default class MeteorApp {
    ...
    /**
     * Injects type="module" into script tags for Meteor 3.x ESM support.
     * This runs unconditionally during the build process.
     */
    injectEsm() {
        this.log.info('injecting ESM support');
        const indexHtml = this.$.env.paths.electronApp.meteorAppIndex;
        let content;
        try {
            content = fs.readFileSync(indexHtml, 'UTF-8');
        } catch (e) {
            this.log.error(`error loading index.html for ESM injection: ${e.message}`);
            process.exit(1);
        }
        ...
        try {
            ...
            this.log.error(`error writing index.html after ESM injection: ${e.message}`);
            process.exit(1);
        }
        this.log.info('ESM support injected successfully');
    }
