...
`;
     ...
     * ROOT_URL and DDP_DEFAULT_CONNECTION_URL before packToAsar seals it
     * into meteor.asar. Catches the 'http://build/' placeholder Meteor emits
     * when the inner 'meteor run' was contended by a leftover meteor-desktop
     * or rspack watcher and acquireIndex() got back half-initialised HTML
     * (seed e2e-4193). Without this gate, packToAsar would seal the placeholder
     * and Electron's sockjs client fails at runtime with
     * 'GET http://build/sockjs/info net::ERR_NAME_NOT_RESOLVED'.
     *
     ...
     *
     * @throws {Error} if either URL is missing, unparseable, has hostname
     *   'build', or (when --ddpUrl was supplied) does not match the
     *   configured ddpUrl.
     */
    validateRuntimeConfigUrls() {
        ...
        ['ROOT_URL', 'DDP_DEFAULT_CONNECTION_URL'].forEach((key) => {
            ...
            try {
                ...
                fail(`${key} is not a parseable URL: ${e.message}`);
            }
            if (url.hostname === 'build') {
                fail(`${key} hostname is 'build' (Meteor's "no --server provided" placeholder)`);
            }
            if (expectedUrl !== null && value !== expectedUrl) {
                fail(`${key}=${value} does not match configured --ddpUrl=${expectedUrl}`);
            }
        });
