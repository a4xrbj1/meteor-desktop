...
export default class MeteorApp {
    ...
    async acquireManifest() {
        ...
        const res = await fetch(
            `http://127.0.0.1:${port}/manifest.json?meteor_dont_serve_index=true`
        );
        const text = await res.text();
    ...
    async copyBuild() {
        ...
        shell.cp(
            '-R', `${buildDir}${copyPathPostfix}`, this.$.env.paths.electronApp.meteorApp
        );
        ...
        shell.cp(
            join(__dirname, '..', 'skeleton', 'desktop-hcp.js'),
