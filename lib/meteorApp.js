...
`;
            ...
            if (missingSrcs.length > 0) {
                ...
                if (missingSrcs.length === 1 && bundleCandidates.length === 1) {
                    // Unambiguous 1-to-1 mapping: rewrite the stale hash to the real file.
                    // (There may be additional unreferenced non-bundle files like desktop-hcp.js —
                    // those are excluded from candidates and left untouched.)
                    const staleSrc = missingSrcs[0];
                    const realFile = bundleCandidates[0];
