...
/**
 * Simple wrapper for fs.rmSync with additional retries in case of failure.
 * It is useful when something is concurrently reading the dir you want to remove.
 */
export function rmWithRetries(...args) {
    // args is ('-rf', path) or just (path) — always take the last argument as the target path
...
export function readFilesAndComputeHash(dir, fileFilter) {
    return new Promise((resolve, reject) => {
        getFileList(dir, true)
            .catch(reject)
            .then((files) => readAndHashFiles(/** @type {any} */ (files), fileFilter))
            .catch(reject)
            .then((result) => {
