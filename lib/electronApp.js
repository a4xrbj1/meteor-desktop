...
import ElectronAppScaffold from './electronAppScaffold.js';
import DependenciesManager from './dependenciesManager.js';
import BinaryModuleDetector from './binaryModulesDetector.js';
...
export default class ElectronApp {
            ...
            asar.createPackage(
                ...
                .then(() => {
                    ...
                    if (extracted) {
                        ...
                        extract.forEach((module) => fs.cpSync(
                        ...
                        if (this.$.utils.exists(
                            ...
                            const binSrc = this.$.env.paths.electronApp.extractedNodeModulesBin;
                            const binDest = path.join(this.$.env.paths.electronApp.nodeModules, '.bin');
                            fs.readdirSync(binSrc).forEach((item) => {
                                fs.cpSync(path.join(binSrc, item), path.join(binDest, item));
                            });
                        }
                    }
    ...
    extractModules(extract) {
        ...
        if (extract.length > 0) {
            if (this.$.utils.exists(this.$.env.paths.electronApp.extractedNodeModules)) {
                fs.rmSync(this.$.env.paths.electronApp.extractedNodeModules, {
                    recursive: true, force: true, maxRetries: 3, retryDelay: 150
                });
            }
     ...
     */
    getModulesToExtract() {
        const binaryModulesDetector = new BinaryModuleDetector(this.$.env.paths.electronApp.nodeModules);
    ...
    async build(run = false) {
        ...
        if (!this.$.desktop.check()) {
            if (!this.$.env.options.scaffold) {
                ...
                // A refusal is a failure, and a script driving this has to be able to see it.
                // The old exit(0) avoided npm printing its own error block on top; measured on
                // npm 12.0.2, a failing script prints two `npm notice` lines and no error block.
                process.exit(1);
            } else {
        ...
        // Transpile before packing so skeleton app files are transpiled into app.asar.
        // packSkeletonToAsar deletes the app/ dir after packing; running transpileAndMinify
        // first ensures appRoot is still present when readDir is called.
        try {
        ...
        // Discover expected files from desktopTmp before packing deletes it.
        let discoveredDesktopFiles = [];
        try {
            discoveredDesktopFiles = this.discoverDesktopAsarFiles();
            this.log.verbose(`discovered ${discoveredDesktopFiles.length} files in .desktop for asar validation`);
        } catch (e) {
            this.log.warn('could not discover desktop files for asar validation: ', e);
        }

        // Flush non-JS files to disk before packing. Files copied via fs.cpSync may
        // still be buffered by the OS when @electron/asar reads them, producing null bytes.
        try {
            this.flushDesktopTmpFiles();
        } catch (e) {
            this.log.warn('could not flush desktop tmp files before packing: ', e);
        }
        ...
        try {
            this.validateDesktopAsar(discoveredDesktopFiles);
        } catch (e) {
            this.log.error('desktop.asar validation failed: ', e);
            process.exit(1);
        }
        ...
        // A7: Build summary gate — log which validation gates passed.
        if (this.validationGatesPassed.length > 0) {
            this.log.info(`A7 build summary: gates passed: ${this.validationGatesPassed.join(', ')}`);
        } else {
            this.log.warn('A7 build summary: no validation gates were recorded');
        }
    ...
    async linkNpmPackages() {
        ...
        if ('linkPackages' in this.$.desktop.getSettings()) {
            if (Array.isArray(settings.linkPackages)) {
                settings.linkPackages.forEach((packageName) => promises.push(
    ...
    checkPluginsVersion(pluginsVersions) {
        const settingsJson = JSON.parse(
            fs.readFileSync(path.join(this.$.env.paths.scaffold, 'settings.json'), 'utf-8')
        );
    ...
    updateDependenciesList() {
        ...
        Object.keys(desktopDependencies.modules).forEach((module) => this.depsManager.mergeDependencies(
            `module[${module}]`,
    ...
    installLocalNodeModules(arch = this.$.env.options.ia32 || process.arch === 'ia32' ? 'ia32' : 'x64') {
        const localDependencies = Object.values(this.packageJson.localDependencies);
    ...
    updatePackageJsonFields() {
        ...
        if ('packageJsonFields' in settings) {
            Object.assign(packageJson, settings.packageJsonFields);
        }
        Object.assign(packageJson, { name: settings.projectName });
    ...
    async updateSettingsJsonFields() {
        ...
        // In dev mode, pass the Meteor dev server port so the skeleton can proxy
        // JS file requests directly to the running Meteor server (which holds
        // individual package files in memory that don't exist on disk).
        if (settings.env !== 'prod') {
            settings.meteorDevServerPort = this.$.env.options.port || 3080;
        }
    ...
    /**
     * Re-reads and re-writes all non-JS files in desktopTmp to ensure OS page-cache
     * writes are committed to disk before @electron/asar reads them for packing.
     * Files copied via fs.cpSync may still be buffered when asar reads them,
     * causing null-byte corruption in the resulting archive.
     */
    flushDesktopTmpFiles() {
        this.log.info('flushing non-JS files in desktopTmp before packing');
        const tmpRoot = this.$.env.paths.desktopTmp.root;
        ...
        const flush = (dir) => {
            ...
            fs.readdirSync(dir, { withFileTypes: true }).forEach((dirent) => {
                ...
                if (dirent.isDirectory()) {
                    ...
                    this.log.debug(`flushed ${path.relative(tmpRoot, full)}`);
                }
            });
        };
    ...
    /**
     * Discovers files in the desktopTmp directory that should be present in desktop.asar.
     * Scans all files recursively and returns relative paths (using forward slashes).
     * Call this before packDesktopToAsar() while desktopTmp still exists.
     *
     * @returns {Array<String>} - list of relative file paths discovered from source .desktop
     */
    discoverDesktopAsarFiles() {
        const tmpRoot = this.$.env.paths.desktopTmp.root;
        const discovered = [];
        ...
        const scanDir = (dir) => {
            ...
            fs.readdirSync(dir, { withFileTypes: true }).forEach((dirent) => {
                ...
                if (dirent.isDirectory()) {
                    ...
                    discovered.push(path.relative(tmpRoot, full).replace(/\\/g, '/'));
                }
            });
        };
    ...
    /**
     * Validates that desktop.asar contains required files after packing.
     * Always checks settings.json and desktop.js. Also validates any
     * dynamically discovered files passed in via the discoveredFiles parameter.
     * Throws an error if any required file is missing so the build fails early.
     *
     * @param {Array<String>} discoveredFiles - files discovered from source .desktop before packing
     */
    validateDesktopAsar(discoveredFiles = []) {
        const asarPath = this.$.env.paths.electronApp.desktopAsar;
        this.log.info('validating desktop.asar contents');
        let files;
        try {
            files = asar.listPackage(asarPath, { isPack: false });
        } catch (e) {
            throw new Error(`failed to read desktop.asar for validation: ${e}`);
        }

        // Normalize asar paths — @electron/asar prefixes entries with '/'
        const normalizedAsarFiles = new Set(files.map((f) => f.replace(/^\//, '')));

        // Always-required files
        const alwaysRequired = ['settings.json', 'desktop.js'];

        // Merge with dynamically discovered files (deduped via Set)
        const allRequired = [...new Set([...alwaysRequired, ...discoveredFiles])];

        const missing = allRequired.filter((file) => !normalizedAsarFiles.has(file));
        if (missing.length > 0) {
            throw new Error(`desktop.asar validation failed: missing files: ${missing.join(', ')}`);
        }
        this.log.verbose(`desktop.asar validation passed (${allRequired.length} files verified)`);

        // A4: Validate settings.json has valid JSON content.
        let settingsContent;
        try {
            settingsContent = asar.extractFile(asarPath, 'settings.json');
        } catch (e) {
            throw new Error(`desktop.asar validation failed: could not extract settings.json: ${e}`);
        }
        let parsedSettings;
        try {
            parsedSettings = JSON.parse(settingsContent.toString('utf8'));
        } catch {
            throw new Error('desktop.asar validation failed: settings.json is not valid JSON');
        }
        if (!parsedSettings || typeof parsedSettings !== 'object') {
            throw new Error('desktop.asar validation failed: settings.json is not valid JSON');
        }

        // A4: Validate desktop.js is non-empty.
        let desktopJsContent;
        try {
            desktopJsContent = asar.extractFile(asarPath, 'desktop.js');
        } catch (e) {
            throw new Error(`desktop.asar validation failed: could not extract desktop.js: ${e}`);
        }
        if (!desktopJsContent || desktopJsContent.toString('utf8').trim().length === 0) {
            throw new Error('desktop.asar validation failed: desktop.js is empty');
        }

        this.log.verbose('A4 desktop.asar content validation passed: settings.json valid, desktop.js non-empty');
        this.validationGatesPassed.push('A4 desktop.asar content');
    }
    ...
    copyDesktopToDesktopTemp() {
        ...
        fs.cpSync(this.$.env.paths.desktop.root, this.$.env.paths.desktopTmp.root, { recursive: true });
        // Remove test files.
        ...
        walkAndDelete(this.$.env.paths.desktopTmp.root);
    }
    ...
    async transpileAndMinify() {
        ...
        const presetEnvResolver = (presetEnv && typeof presetEnv === 'object' && presetEnv.default)
            ? presetEnv.default
            : presetEnv;

        const uglifyingEnabled = 'uglify' in settings && !!settings.uglify;

        const desktopFiles = fs.existsSync(this.$.env.paths.desktopTmp.root)
            ? (await this.$.utils.readDir(this.$.env.paths.desktopTmp.root)).data
            : [];
        const skeletonAppFiles = fs.existsSync(this.$.env.paths.electronApp.appRoot)
            ? (await this.$.utils.readDir(this.$.env.paths.electronApp.appRoot)).data
            : [];
        const files = desktopFiles.concat(skeletonAppFiles);
        ...
        for (const file of files) {
            if (file.endsWith('.js')) {
                let { code } = transformFileSync(file, {
                    babelrc: false,
                    configFile: false,
                    presets: [[presetEnvResolver, { targets: { node: 'current' }, modules: 'commonjs' }]]
                });

                code = code
                    .replace(/import\.meta\.url/g, '__filename')
                    .replace(/fileURLToPath\(__filename\)/g, '__filename')
                    .replace(/\(0,\s*[_$a-zA-Z0-9]+\.fileURLToPath\)\(__filename\)/g, '__filename');

                if (settings.env === 'prod' && uglifyingEnabled) {
                    try {
                        // eslint-disable-next-line no-await-in-loop
                        const result = await uglify.minify(code, options);
                        code = result.code;
                    } catch (e) {
                        throw new Error(e);
                    }
    ...
    async excludeFilesFromArchive() {
        ...
        fs.mkdirSync(this.$.env.paths.electronApp.extracted);
        ...
        configs.forEach((config) => {
            ...
            if ('extract' in moduleConfig) {
                ...
                moduleConfig.extract.forEach((file) => {
                    ...
                    fs.renameSync(filePath, path.join(destinationPath, path.basename(filePath)));
                });
            }
        });
    }
