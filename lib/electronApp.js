...
export default class ElectronApp {
    ...
    async transpileAndMinify() {
        ...
        const packageJson = JSON.parse(fs.readFileSync(path.join(this.$.env.paths.meteorDesktop.root, 'package.json'), 'utf-8'));

        const preset = presetEnv({
            version: packageJson.dependencies['@babel/preset-env'],
            assertVersion: () => { },
            caller: (cb) => cb({ name: 'meteor-desktop' })
        }, { targets: { node: '14' } });
