...
const meteorDesktopVersion = JSON.parse(fs.readFileSync(path.resolve(path.join(currentDir, '..', '..', 'package.json')), 'utf-8')).version;
...
function initTestsSupport() {
    log('installing cross-env and ava');
    log('running `meteor npm install --save-dev cross-env ava`');

    let installFailed = false;
    try {
        const cmd = 'meteor npm install --save-dev cross-env ava';
    ...
    if (installFailed) {
        warn('could not add cross-env and ava to your `devDependencies`, please do it'
            + ' manually');
    }
...
if (process.argv.length === 2 || !~('-h|--help|run|init|build|build-installer|just-run|init-tests-support|package'
    .indexOf(cmd))
) {
    let { argv } = process;
