import path from 'path';
import fs from 'fs';
import { createRequire } from 'module';
import spawn from 'cross-spawn';
import Log from './log.js';
import defaultDependencies from './defaultDependencies.js';

const require = createRequire(import.meta.url);

/**
 * Removes a directory recursively with an optional delay.
 *
 * @param {string} dirPath - path to the dir to be deleted
 * @param {number} delay - delay the task by ms
 * @returns {Promise<any>}
 */
function removeDir(dirPath, delay = 0) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            fs.promises.rm(dirPath, { recursive: true, force: true })
                .then(() => {
                    resolve();
                })
                .catch((err) => {
                    reject(err);
                });
        }, delay);
    });
}

/**
 * Wrapper for electron-builder.
 */
export default class InstallerBuilder {
    /**
     * @param {MeteorDesktop} $ - context
     *
     * @constructor
     */
    constructor($) {
        this.log = new Log('electronBuilder');
        this.$ = $;
        this.firstPass = true;
        this.lastRebuild = {};
        this.currentContext = null;
        this.installerDir = path.join(this.$.env.options.output, this.$.env.paths.installerDir);
        this.platforms = [];
        this.nodeModulesMovedOut = false;
    }

    async init() {
        this.builder = await this.$.getDependency('electron-builder', defaultDependencies['electron-builder']);
        // FLAGGED, NOT FIXED: this reads the `electron-builder` pin for `app-builder-lib`. Inert only
        // because the two values in defaultDependencies.js are equal (26.15.3); the day they diverge
        // this installs — or warns about — the wrong one. Left alone deliberately rather than
        // changed as a drive-by; see the CHANGELOG entry for the pin refresh.
        const appBuilder = await this.$.getDependency('app-builder-lib', defaultDependencies['electron-builder'], false);

        this.yarn = require(path.join(appBuilder.path, 'out', 'util', 'yarn'));
        this.getGypEnv = this.yarn.getGypEnv;
        this.packageDependencies = require(path.join(appBuilder.path, 'out', 'util', 'packageDependencies'));
    }

    /**
     * Prepares the last rebuild object for electron-builder.
     *
     * @param {string} arch
     * @param {string} platform
     * @returns {Object}
     */
    prepareLastRebuildObject(arch, platform = process.platform) {
        this.lastRebuild = {
            frameworkInfo: { version: this.$.getElectronVersion(), useCustomDist: true },
            platform,
            arch
        };
        // app-builder-lib < 26 consumed a precomputed productionDeps array built here via
        // packageDependencies.createLazyProductionDeps(). 26.x removed that export (the module
        // is now empty) and collects production deps internally via node-module-collector, and
        // its installOrRebuild/rebuild (out/util/yarn.js) no longer reads productionDeps at all.
        // Only set the field when the legacy helper is present so both lines keep working.
        if (typeof this.packageDependencies.createLazyProductionDeps === 'function') {
            this.lastRebuild.productionDeps = this.packageDependencies
                .createLazyProductionDeps(this.$.env.paths.electronApp.root);
        }
        return this.lastRebuild;
    }

    /**
     * Calls npm rebuild from electron-builder.
     * @param {string} arch
     * @param {string} platform
     * @param {boolean} install
     * @returns {Promise}
     */
    async installOrRebuild(arch, platform = process.platform, install = false) {
        this.log.debug(`calling installOrRebuild from electron-builder for arch ${arch}`);
        this.prepareLastRebuildObject(arch, platform);
        await this.yarn.installOrRebuild(this.$.desktop.getSettings().builderOptions || {},
            {
                appDir: this.$.env.paths.electronApp.root,
                projectDir: this.$.env.paths.electronApp.root,
                workspaceRoot: null
            },
            this.lastRebuild,
            install,
            process.env);
    }

    /**
     * Callback invoked before build is made. Ensures that app.asar have the right rebuilt
     * node_modules.
     *
     * @param {Object} context
     * @returns {Promise}
     */
    beforeBuild(context) {
        this.currentContext = { ...context };
        return new Promise((resolve, reject) => {
            const platformMatches = process.platform === context.platform.nodeName;
            const rebuild = platformMatches && context.arch !== this.lastRebuild.arch;
            if (!platformMatches) {
                this.log.warn('skipping dependencies rebuild because platform is different, if you have native '
                    + 'node modules as your app dependencies you should od the build on the target platform only');
            }

            if (!rebuild) {
                this.moveNodeModulesOut()
                    .catch((e) => reject(e))
                    .then(() => setTimeout(() => resolve(false), 2000));
                // Timeout helps on Windows to clear the file locks.
            } else {
                // Lets rebuild the node_modules for different arch.
                this.installOrRebuild(context.arch, context.platform.nodeName)
                    .catch((e) => reject(e))
                    .then(() => this.$.electronApp.installLocalNodeModules(context.arch))
                    .catch((e) => reject(e))
                    .then(() => {
                        this.$.electronApp.scaffold.createAppRoot();
                        this.$.electronApp.scaffold.copySkeletonApp();
                        return this.$.electronApp.transpileAndMinify()
                            .then(() => this.$.electronApp.packSkeletonToAsar(
                                [
                                    this.$.env.paths.electronApp.meteorAsar,
                                    this.$.env.paths.electronApp.desktopAsar,
                                    this.$.env.paths.electronApp.extracted
                                ]
                            ));
                    })
                    .catch((e) => reject(e))
                    .then(() => this.moveNodeModulesOut())
                    .catch((e) => reject(e))
                    .then(() => resolve(false));
            }
        });
    }

    /**
     * Callback to be invoked after packing. Restores node_modules to the .desktop-build.
     * @returns {Promise}
     */
    afterPack(context) {
        this.platforms = this.platforms
            .filter((platform) => platform !== context.electronPlatformName);
        if (this.platforms.length !== 0) {
            return Promise.resolve();
        }
        return new Promise((resolve, reject) => {
            if (this.$.utils.exists(this.$.env.paths.electronApp.extractedNodeModules)) {
                this.log.debug('injecting extracted modules');
                const extractedSrc = this.$.env.paths.electronApp.extractedNodeModules;
                const destNodeModules = path.join(this.getPackagedAppPath(context), 'node_modules');
                fs.readdirSync(extractedSrc).forEach((item) => {
                    fs.cpSync(path.join(extractedSrc, item), path.join(destNodeModules, item), { recursive: true });
                });
            }

            this.log.debug('moving node_modules back');
            // Move node_modules back.

            try {
                this.restoreNodeModules();
            } catch (e) {
                reject(e);
                return;
            }

            if (this.firstPass) {
                this.firstPass = false;
            }
            this.log.debug('node_modules moved back');

            this.wait()
                .catch((e) => reject(e))
                .then(() => resolve());
        });
    }

    /**
     * This command kills orphaned MSBuild.exe processes.
     * Sometime after native node_modules compilation they are still writing some logs,
     * prevent node_modules from being deleted.
     */
    killMSBuild() {
        if (this.currentContext.platform.nodeName !== 'win32') {
            return;
        }
        try {
            const out = spawn
                .sync(
                    'wmic',
                    ['process', 'where', 'caption="MSBuild.exe"', 'get', 'processid']
                )
                .stdout.toString('utf-8')
                .split('\n');

            const regex = /(\d+)/gm;
            // No we will check for those with the matching params.
            out.forEach((line) => {
                const match = regex.exec(line) || false;
                if (match) {
                    this.log.debug(`killing MSBuild.exe at pid: ${match[1]}`);
                    spawn.sync('taskkill', ['/pid', match[1], '/f', '/t']);
                }
                regex.lastIndex = 0;
            });
        } catch {
            this.log.debug('kill MSBuild failed');
        }
    }

    /**
     * Returns the path to packaged app.
     * @returns {string}
     */
    getPackagedAppPath(context = {}) {
        if (this.currentContext.platform.nodeName === 'darwin') {
            return path.join(
                this.installerDir,
                'mac',
                `${context.packager.appInfo.productFilename}.app`,
                'Contents',
                'Resources',
                'app'
            );
        }
        const archSuffix = this.currentContext.arch === 'ia32' ? 'ia32-' : '';
        const osPart = this.currentContext.platform.nodeName === 'win32' ? 'win' : 'linux';
        const platformDir = `${osPart}-${archSuffix}unpacked`;
        return path.join(
            this.installerDir,
            platformDir,
            'resources',
            'app'
        );
    }

    /**
     * On Windows it waits for the app.asar in the packed app to be free (no file locks).
     * @returns {*}
     */
    wait() {
        if (this.currentContext.platform.nodeName !== 'win32') {
            return Promise.resolve();
        }
        const appAsarPath = path.join(
            this.getPackagedAppPath(),
            'app.asar'
        );
        let retries = 0;
        const self = this;
        return new Promise((resolve, reject) => {
            function check() {
                fs.open(appAsarPath, 'r+', (err, fd) => {
                    retries += 1;
                    if (err) {
                        if (err.code !== 'ENOENT') {
                            self.log.debug(`waiting for app.asar to be readable, ${'code' in err ? `currently reading it returns ${err.code}` : ''}`);
                            if (retries < 6) {
                                setTimeout(() => check(), 4000);
                            } else {
                                reject(`file is locked: ${appAsarPath}`);
                            }
                        } else {
                            resolve();
                        }
                    } else {
                        fs.closeSync(fd);
                        resolve();
                    }
                });
            }
            check();
        });
    }

    /**
     * Prepares the target object passed to the electron-builder.
     *
     * @returns {Map<Platform, Map<Arch, Array<string>>>}
     */
    prepareTargets() {
        let arch = this.$.env.options.ia32 ? 'ia32' : 'x64';
        arch = this.$.env.options.allArchs ? 'all' : arch;

        const targets = [];

        if (this.$.env.options.win) {
            targets.push(this.builder.dependency.Platform.WINDOWS);
        }
        if (this.$.env.options.linux) {
            targets.push(this.builder.dependency.Platform.LINUX);
        }
        if (this.$.env.options.mac) {
            targets.push(this.builder.dependency.Platform.MAC);
        }

        if (targets.length === 0) {
            if (this.$.env.os.isWindows) {
                targets.push(this.builder.dependency.Platform.WINDOWS);
            } else if (this.$.env.os.isLinux) {
                targets.push(this.builder.dependency.Platform.LINUX);
            } else {
                targets.push(this.builder.dependency.Platform.MAC);
            }
        }
        return this.builder.dependency.createTargets(targets, null, arch);
    }

    async build() {
        const settings = this.$.desktop.getSettings();
        if (!('builderOptions' in settings)) {
            this.log.error(
                'no builderOptions in settings.json, aborting'
            );
            process.exit(1);
        }

        const builderOptions = { ...settings.builderOptions };

        builderOptions.asar = false;
        builderOptions.npmRebuild = true;

        builderOptions.beforeBuild = this.beforeBuild.bind(this);
        builderOptions.afterPack = this.afterPack.bind(this);
        builderOptions.electronVersion = this.$.getElectronVersion();

        builderOptions.directories = {
            app: this.$.env.paths.electronApp.root,
            output: path.join(this.$.env.options.output, this.$.env.paths.installerDir)
        };

        if ('mac' in builderOptions && 'target' in builderOptions.mac) {
            if (builderOptions.mac.target.includes('mas')) {
                this.platforms = ['darwin', 'mas'];
            }
        }

        // NO try/catch around this. An electron-builder failure must reach the caller:
        // buildInstaller() in lib/index.js logs it and rethrows when asked to, and the CLI asks.
        // This used to catch and only log, which silently outranked all of that plumbing - a
        // Windows installer build that produced no artifacts at all still reported exit 0 to the
        // shell, so a script under `set -e` walked straight past it (seed frontend-4e42). The
        // `throwError` argument added for seed meteor-desktop-a8f8 was inert for this exact path,
        // because the rethrow it controls sits OUTSIDE this catch and never saw an error.
        this.log.debug('calling build from electron-builder');
        try {
            await this.builder.dependency.build({
                targets: this.prepareTargets(),
                config: builderOptions,
                ...settings.builderCliOptions
            });
        } finally {
            // The move out and the move back live in two different electron-builder hooks and only
            // one of them is on the failure path: beforeBuild calls moveNodeModulesOut(), and the
            // rename back happens in afterPack, which a failed build never reaches. afterPack also
            // early-returns for every platform but the last, so even a build that gets that far can
            // fail between two afterPack calls. Without this the app is left with no node_modules
            // at all and the real ones stranded in tmpNodeModules (seed meteor-desktop-701d).
            //
            // On the success path afterPack already restored them and cleared the flag, so this is
            // a no-op there. It never masks the build's own error: the restore's failure is logged
            // rather than thrown, so a `finally` cannot replace the exception on its way out.
            try {
                if (this.restoreNodeModules()) {
                    this.log.debug('node_modules moved back after a failed build');
                }
            } catch (e) {
                this.log.warn('could not move node_modules back into the app - they are left in '
                    + `${this.$.env.paths.electronApp.tmpNodeModules}: `, e);
            }
        }

        // Deleting a temp dir is NOT part of building the installer, so it keeps a catch of its own
        // rather than riding on the one that used to wrap both. Without this scoping, removing that
        // catch would newly report a SUCCESSFUL build as a failed one whenever this rmSync lost a
        // race with a Windows file lock - which is a live risk here, see killMSBuild()/wait(). An
        // adversarial review caught that; the first version of this fix had it wrong.
        try {
            if (this.$.utils.exists(this.$.env.paths.electronApp.extractedNodeModules)) {
                fs.rmSync(this.$.env.paths.electronApp.extractedNodeModules, { recursive: true, force: true, maxRetries: 3 });
            }
        } catch (e) {
            this.log.warn('could not remove the extracted node_modules dir, the installer itself was'
                + ' built fine - remove it by hand if it is in the way: ', e);
        }
    }

    /**
     * Moves node_modules back into the app, undoing moveNodeModulesOut().
     *
     * Safe to call when nothing was moved, and safe to call twice - both are no-ops - because it
     * has two callers: afterPack on the success path, and build()'s finally on the failure path.
     *
     * @returns {Boolean} - True when a move actually happened.
     */
    restoreNodeModules() {
        if (!this.nodeModulesMovedOut) {
            return false;
        }
        fs.renameSync(
            this.$.env.paths.electronApp.tmpNodeModules,
            this.$.env.paths.electronApp.nodeModules
        );
        this.nodeModulesMovedOut = false;
        return true;
    }

    /**
     * Moves node_modules out of the app because while the app will be packaged
     * we do not want it to be there.
     * @returns {Promise<any>}
     */
    async moveNodeModulesOut() {
        this.log.debug('moving node_modules out, because we have them already in'
            + ' app.asar');
        this.killMSBuild();
        await removeDir(this.$.env.paths.electronApp.tmpNodeModules);
        fs.renameSync(
            this.$.env.paths.electronApp.nodeModules,
            this.$.env.paths.electronApp.tmpNodeModules
        );
        this.nodeModulesMovedOut = true;
        await this.wait();
        // A `.catch(e => reject(e)).then(next)` chain used to stand here. Rejecting the outer
        // promise does NOT stop the inner chain, so every later step still ran after a failure -
        // including this delayed removeDir. On a Windows lock that made wait() exhaust, the caller
        // saw the rejection, build()'s finally restored node_modules, and then this fired 1000ms
        // later and deleted what had just been restored. Awaiting is what makes a failure stop
        // here (adversarial review finding P2, reproduced before it was believed).
        await removeDir(this.$.env.paths.electronApp.nodeModules, 1000);
        await this.wait();
    }
}
