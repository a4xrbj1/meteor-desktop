...
Dev-tooling, test, and behavior-preserving lint-cleanup changes. The `typescript` devDep (fa84) and the `tests/` changes (5602, fcde) leave the published artifact **byte-identical**. The lint cleanup (fbee) **does** textually modify shipped `lib/`+`skeleton/` files (style + dead-code removal, behavior preserved), so the published artifact changes — a `npm run desktop` / `desktop build` consumer exercise (Rule 48) is recommended before the release that ships this batch. No version bump / publish yet; fold into the next functional release.
