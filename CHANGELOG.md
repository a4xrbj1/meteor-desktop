...
Dev-tooling-only fix — **no change to the published artifact** (`typescript` is a `devDependency`, absent from `package.json#files`, and `devDependencies` are never installed transitively by consumers, so a `frontend` `npm install @a4xrbj1/meteor-desktop` is byte-identical). No version bump / publish required; fold into the next functional release.
...
* **ESLint runs again.** `npm run lint` (`eslint lib scaffold skeleton tests`) executes to completion instead of crashing; inversion confirmed — removing `typescript` reproduces the `Cannot find module 'typescript'` crash. It now surfaces **101 pre-existing errors / 88 warnings** on untouched source lines — all pre-existing (the diff touches only `package.json` + `package-lock.json`), filed separately per Rule 26 (seed pending), not fixed here.
* **Tests unaffected.** Full suite `209 passing, 1 failing` — the single failure is the pre-existing `meteorApp #validateHashCoherence 'every stylesheet link unresolvable'` test (seeds `meteor-desktop-5602` / `-3eed`), not touched by this change.
* **Rule 48 N/A.** No consumer exercise required: the changed path (eslint config resolution) is dev-machine-only and never reached by `npm run desktop` / `desktop build`; CI (`test.yml`) does not run lint, so CI was never red.
