...
* **A failed installer build no longer strands the app's `node_modules` outside the app** (seed `meteor-desktop-701d`). `beforeBuild` calls `moveNodeModulesOut()`, which renames `electronApp.nodeModules` to `electronApp.tmpNodeModules`; the rename back lived **only** in `afterPack`. Any electron-builder failure between the two hooks — a signing-toolchain refusal, a target it cannot build, an aborted pack — left the app with no `node_modules` at all and the real ones sitting in a temp directory with nothing to restore them. There was no `finally`. A cleanup-parity gap, ENGINEERING.md Rule 40.
  * **Observed, not theorised.** The 2026-08-30 signed `--mac --win` beta run failed in `WindowsSignAzureManager.initialize` and left `.meteor/.desktop_node_modules/` behind in the consumer. That untracked directory is what tripped frontend's provenance re-check and, by accident, stopped the run (seed `frontend-4e42`, which is about the exit code rather than this). The directory is now gitignored there, which removes the false complaint **and** removes the accident that stopped the run — so the gap is fully silent from here on.
  * **`afterPack` is not a plain pair with `beforeBuild`, which is why a bare `finally` around the restore would not have been enough.** It early-returns for every platform but the last (`this.platforms` filtering, for the `darwin`/`mas` pair), so a multi-target build can fail *between* two `afterPack` calls with the modules still out. The restore is therefore gated on a `nodeModulesMovedOut` flag set by the successful rename in `moveNodeModulesOut()` and cleared by whichever caller actually moves them back, rather than on which hook ran.
  * **The `finally` cannot mask the build's own error.** `restoreNodeModules()` is called inside its own `try`/`catch` there; a restore that fails is logged with the path the modules were left at, and the original electron-builder exception propagates untouched. On the success path `afterPack` has already restored and cleared the flag, so the `finally` is a no-op.
  * **Why it had not bitten harder:** `electronApp.build()` regenerates the `.desktop-build` tree ahead of `electronBuilder.build()` on every run, so the next attempt rebuilt what was stranded. That made this a disk-space and confusion bug rather than a broken-retry one — but it depended on a property of a *different* function and nothing stated it.
  * **`moveNodeModulesOut()` no longer keeps running after it has failed** — a defect this fix itself made dangerous, found by the adversarial review, which reproduced it before reporting it. The function was a `.catch((e) => reject(e)).then(next)` chain, and rejecting the outer promise does **not** stop the inner one: after any failure every later step still ran, including a `removeDir(nodeModules, 1000)`. Adding a restore turned that from harmless into destructive — move-out fails on a locked Windows `app.asar`, the build aborts, the `finally` puts `node_modules` back, and a second later the pending cleanup deletes it. Stranded-but-recoverable became *deleted*. It is now a plain `async`/`await` function: a rejection stops there, and it is shorter than the chain it replaces.
  * Three tests on the existing failing-electron-builder harness, all inversion-verified: removing the `finally` leaves the marker file in `tmpNodeModules` and fails the restore assertion; deleting the `nodeModulesMovedOut` guard makes `restoreNodeModules()` throw `ENOENT` where it should return `false` — the case that matters because `afterPack` **rejects** on a throw rather than swallowing it. and putting the promise chain back leaves `node_modules` deleted 1.3 s after a failed move-out. That last one needs no stubbing at all: `tmpNodeModules` is given a parent that does not exist, so `renameSync` throws `ENOENT` while `node_modules` is still populated. A fourth, weaker test was written first and discarded: it asserted the no-op through `build()`, where the `finally`'s own `catch` absorbed the `ENOENT`, so it passed with and without the guard. 330 passing, eslint and tsc clean.
