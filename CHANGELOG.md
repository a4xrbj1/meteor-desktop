...
Dev-tooling, test, lint-cleanup, **and one functional feature** (the e490 web-HCP revival). The `typescript` devDep (fa84) and the `tests/` changes (5602, fcde) leave the published artifact **byte-identical**. The lint cleanup (fbee) and the **e490** feature **do** textually modify shipped `skeleton/` files. e490 is a behavioral change to the renderer HCP bridge + the bundle integrity check, so a `npm run desktop` / `desktop build` consumer exercise (Rule 48) is **required** before the release that ships this batch. No version bump / publish yet; fold into the next functional release.
