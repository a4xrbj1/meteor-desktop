...
Dev-tooling / test-only changes — **no change to the published artifact** (touched: `package.json` devDep + lockfile, and `tests/`; none are in `package.json#files`, and `devDependencies` / test files are never installed transitively by consumers, so a `frontend` `npm install @a4xrbj1/meteor-desktop` is byte-identical). No version bump / publish required; fold into the next functional release.
...
* **Full suite now `210 passing, 0 failing`** (was `209 passing, 1 failing`; the `validateHashCoherence` fix above closes the last failure). Inversion (Rule 41): reverting the mock reproduces the `TypeError`; the green proves the code reaches the intended throw at `meteorApp.js:1157` — the `/style-less desktop build/` message exists nowhere else.
* **Rule 48 N/A.** No consumer exercise required: the changed paths (eslint config resolution; a unit-test mock) are dev-machine-only and never reached by `npm run desktop` / `desktop build`; CI (`test.yml`) does not run lint, so CI was never red.
